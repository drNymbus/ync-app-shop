...
function BasketItem({ basket, id, compact, add, rm, del }) {
	...
	return (<>
		{Object.entries(basket[id]).map((el) => {
			...
			return (
				<BasketArticle
					key={size} item={item} size={size} quantity={basket[id][size]}
					max_qtty={quantity?.[size]} compact={compact}
					add={add} rm={rm} del={del}
				/>
			);
		})}
	</>);
	...
	const emptyBasket = (
		<div className="empty-basket-wrapper">
			...
			<div className="empty-basket-image-container">
				<img src="/assets/empty_basket3.svg" className="empty-basket-image"/>
			</div>
		</div>
	);
			...
			<div className="basket">
				<div className="basket-rows">
					{Object.keys(basket).map((item, i) => (
						<BasketItem key={i} basket={basket} id={item} compact={compact} add={add} rm={rm} del={del}/>
					))}
				</div>
